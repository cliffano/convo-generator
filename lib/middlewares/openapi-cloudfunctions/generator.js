const convo = require('convo-node');
const fs = require('fs');

/**
 * Parse additional arguments and initialise configurations.
 *
 * @param {Object} generator: the Convo generator object
 */
function init(generator) {
  generator.argument('convoSpec', { type: String, required: true });
  generator.argument('openApiSpec', { type: String, required: true });

  const convoSpecString = fs.readFileSync(generator.options.convoSpec, 'utf8');
  const openApiSpecString = fs.readFileSync(generator.options.openApiSpec, 'utf8');
  generator.config = new convo.openApiCloudFunctions.Config(convoSpecString, openApiSpecString);
  generator.params = {
    config: generator.config
  };
}

/**
 * Generate middleware files from EJS templates.
 *
 * @param {Object} generator: the Convo generator object
 */
function generate(generator) {
  generator.log('Generating Convo OpenAPI3-CloudFunctions Middleware templates for %s specification', generator.config.convoSpec.info.title);

  ['.gitignore', 'index.js', 'package.json', 'serverless.yml'].forEach(function (fileName) {
    generator.fs.copyTpl(
      generator.templatePath('middlewares/openapi-cloudfunctions/' + fileName + '.ejs'),
      generator.destinationPath(fileName),
      generator.params
    );
  });
}

module.exports = {
  init: init,
  generate: generate
}
